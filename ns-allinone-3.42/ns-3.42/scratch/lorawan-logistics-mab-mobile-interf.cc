/*
 * Simulation LoRaWAN logistique mobile avec interférences (bâtiments, vent, arbres, réseau voisin)
...
struct ReceptionLog {
    ...
    double timeOnAir;
    uint32_t gatewayId;
    double interferenceLoss; // perte due aux interférences
};
...
public:
    ...
    void SendPacket() {
        ...
        ReceptionLog log = {m_deviceId, static_cast<uint32_t>(m_totalTx+1), now, pos.x, pos.y, pos.z, distance, m_txPower, m_sf, m_bw, m_payload, m_cr, rssi, snr, success, m_energyConsumed, timeOnAir, 1, interfLoss};
...
int main(int argc, char *argv[]) {
    std::cout << "[SIMU] Début simulation LoRaWAN logistique mobile avec interférences" << std::endl;
    // Création du dossier de résultats si besoin
    std::string resultsDir = "lorawan_mobile_results_interf";
    struct stat st = {0};
    ...
    uint32_t nDevices = 1000; // 100 devices pour 100 combinaisons
    int cr = 1;
    ...
    for (uint32_t i = 0; i < nDevices; ++i) {
        ...
        int txPower = txPowerList[(i / (sfList.size()*payloadList.size()*bwList.size())) % txPowerList.size()];
        int payload = payloadList[(i / (sfList.size()*txPowerList.size()*bwList.size())) % payloadList.size()];
        int bw = bwList[(i / (sfList.size()*txPowerList.size()*payloadList.size())) % bwList.size()];
    ...
    fname << resultsDir << "/lorawan-logistics-mab-mobile_interf_ALL.csv";
    std::ofstream out;
...
out << log.deviceId << "," << log.messageId << "," << buffer << "," << log.x << "," << log.y << "," << log.z << "," << log.distance << "," << log.txPower << "," << log.sf << "," << log.bw << "," << log.cr << "," << log.payload << "," << nDevices << "," << log.rssi << "," << log.snr << "," << log.success << "," << log.energyConsumed << "," << log.timeOnAir << "," << totalTx << "," << totalRx << "\n";
    ...
    out.close();
    Simulator::Destroy();
    std::cout << "[SIMU] Fin simulation LoRaWAN logistique mobile avec interférences" << std::endl;
    return 0;
}
