/*
 * Simulation LoRaWAN logistique statique avec interférences (bâtiments, vent, arbres, pluie, réseau voisin)
...
public:
    static TypeId GetTypeId(void) {
        static TypeId tid = TypeId("LogisticsGatewayInterfStatic")
            .SetParent<Object>()
    ...
    // Ajout d'un paramètre d'interférence, txPowerDbm: puissance d'émission, sf: spreading factor, bw: bandwidth
    bool ReceivePacket(uint32_t deviceId, const Vector& devicePosition, double txPowerDbm, int sf, int bw, double& rssi, double& snr, double& interfLoss) {
        ...
        interfLoss = 0.0;
        // Bâtiments : perte supplémentaire aléatoire 0-10 dB
        interfLoss += m_rng->GetValue(0.0, 10.0);
        // Vent : perte supplémentaire 0-2 dB
        interfLoss += m_rng->GetValue(0.0, 2.0);
        // Arbres : perte supplémentaire 0-5 dB
        interfLoss += m_rng->GetValue(0.0, 5.0);
        // Pluie : perte supplémentaire 0-6 dB
        interfLoss += m_rng->GetValue(0.0, 6.0);
        // Réseau voisin : perte supplémentaire 0-8 dB
        interfLoss += m_rng->GetValue(0.0, 8.0);
...
public:
    ...
    static TypeId GetTypeId(void) {
        static TypeId tid = TypeId("LogisticsEndDeviceInterfStatic")
            .SetParent<Object>()
            .AddConstructor<LogisticsEndDevice>();
        return tid;
    }
    LogisticsEndDevice() : m_deviceId(0), m_txPower(14.0), m_energyConsumed(0.0), m_totalTx(0), m_totalRx(0), m_paramIndex(0) {
        m_rng = CreateObject<UniformRandomVariable>();
    }
    ...
    void SetParamCombos(const std::vector<Params>& combos) {
        ...
        m_paramCounts = std::vector<uint32_t>(combos.size(), 0); // Compteur d'utilisation
    }
    ...
    void SendPacket() {
        // Phase d'exploration systématique : chaque combinaison testée au moins une fois
        uint32_t idx = 0;
        if (!m_paramCombos.empty()) {
            auto it = std::find(m_paramCounts.begin(), m_paramCounts.end(), 0);
            if (it != m_paramCounts.end()) {
                // Il reste au moins une combinaison jamais testée
                idx = std::distance(m_paramCounts.begin(), it);
            } else {
                // Toutes les combinaisons ont été testées au moins une fois : epsilon-greedy
        ...
        ReceptionLog log = {m_deviceId, static_cast<uint32_t>(m_totalTx+1), now, pos.x, pos.y, pos.z, distance, m_txPower, m_sf, m_bw, m_payload, m_cr, rssi, snr, success, m_energyConsumed, timeOnAir, interfLoss};
...
private:
    ...
    std::vector<uint32_t> m_paramCounts; // Compteur d'utilisation pour chaque combinaison
    uint32_t m_paramIndex = 0;
    double m_epsilon = 0.1;
    uint32_t m_lastParamUsed = 0;
};

int main(int argc, char *argv[]) {
    std::cout << "[SIMU] Début simulation LoRaWAN logistique statique avec interférences (EU868 + permutations cycliques)" << std::endl;
    ...
    uint32_t nMessages = 20;  // 100 messages par dispositif
    double simTime = nMessages * packetIntervalSeconds;
    double areaRadius = 1000.0;
    std::string outDir = "lorawan_static_results_interf";
    mkdir(outDir.c_str(), 0777);
    // Génération de toutes les combinaisons
    std::vector<LogisticsEndDevice::Params> allCombos;
    for (int sf : sfList)
        for (int txp : txPowerList)
            for (int pl : payloadList)
                for (int bw : bwList)
                    allCombos.push_back({sf, txp, pl, bw});
    // Fréquences EU868
    ...
    for (uint32_t i = 0; i < nDevices; ++i) {
        ...
        dev->Setup(i + 1, gateway, mob, (double)txPowerList[0]);
        dev->SetLoRaParams(sfList[0], bwList[0], cr, payloadList[0]);
    ...
    fname << outDir << "/lorawan-static-interf_ALL.csv";
    std::ofstream out;
    ...
    if (writeHeader) {
        out << "deviceId,messageId,time,x,y,z,distance,txPower,sf,bw,cr,payload,nDevices,rssi,snr,success,energyConsumed,timeOnAir,interferenceLoss,totalTx,totalRx\n";
    }
...
out << log.deviceId << "," << log.messageId << "," << buffer << "," << log.x << "," << log.y << "," << log.z << "," << log.distance << "," << log.txPower << "," << log.sf << "," << log.bw << "," << log.cr << "," << log.payload << "," << nDevices << "," << log.rssi << "," << log.snr << "," << log.success << "," << log.energyConsumed << "," << log.timeOnAir << "," << log.interferenceLoss << "," << totalTx << "," << totalRx << "\n";
        }
    }
    out.close();
    Simulator::Destroy();
    std::cout << "[SIMU] Fin simulation LoRaWAN logistique statique avec interférences (EU868 + permutations cycliques)" << std::endl;
    return 0;
}
