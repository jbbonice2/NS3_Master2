...
public:
    ...
    void SendPacket() {
        ...
        // Probabilité de succès plus élevée en milieu rural (moins d'obstacles)
        // SF plus élevé = meilleure sensibilité = meilleure probabilité de succès à longue distance
        double successProbability = 0.80 + (0.03 * spreadingFactorIndex);
        
        // Limiter à 95% max
        successProbability = std::min(successProbability, 0.95);
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Creating network with " << nEndDevices << " devices." << std::endl;
    ...
    for (int i = 0; i < nEndDevices; ++i)
    {
        ...
        mac->SetRange(10000.0 + i * 1000.0); // 10-14 km (rural)
